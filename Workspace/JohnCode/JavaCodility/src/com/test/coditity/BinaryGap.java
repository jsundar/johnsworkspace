 ...
 * Write a function: 
 * class Solution { public int solution(int N); } that, given a positive integer N, returns the length of its longest binary gap. 
...
public class BinaryGap {
	public static String getLongestBinaryGap(int num) {
		
		String binary = Integer.toBinaryString(num);
		System.out.println("After converting Binary : " + binary);
		StringTokenizer token = new StringTokenizer(binary,"1");
		String[] a = new String[token.countTokens()];
		int i = 0;
		int biglength = 0;
		while(token.hasMoreTokens()) {
			a[i] = token.nextToken();
			if(a[i].length() > biglength) {
				biglength = a[i].length();
			}
		}
		...
		return String.valueOf(biglength);
		
	}

	public static void main(String[] args) {
		System.out.println(getLongestBinaryGap(1703));
		System.out.println(getLongestBinaryGap(529));
		System.out.println(getLongestBinaryGap(9));
	}
}
